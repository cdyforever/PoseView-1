    ...
    def put_vectormap(vectormap, countmap, plane_idx, center_from, center_to, threshold=8):
        ...
        for y in range(min_y, max_y):
            for x in range(min_x, max_x):
                ...
                #((x - x10)*(x21 - x11) - (x20 - x10)*(y - x11))^2 <= 64
                if dist > threshold:
                    continue
...
class CocoPose(RNGDataFlow):
    ...
    def __init__(self, path, img_path=None, is_train=True, decode_img=True, only_idx=-1):
        ...
        self.img_path = (img_path if img_path is not None else '') + ('images/train2014/' if is_train else 'images/val2014/')
...
if __name__ == '__main__':
    ...
    df = _get_dataflow_onlyread('/media/senya/data1/dataset/COCO/annotations', True, '/media/senya/data1/dataset/COCO/')
